    ...
    class DashboardController {
        ...
        */
        private function docenteDashboard() {
            ...
            // 1. RECUPERACIÓN INTELIGENTE DE IDENTIDAD
            // Consultamos al sistema Auth (users.db) para obtener los datos completos del usuario logueado.
            // Esto es crucial porque el 'usuario' de login puede ser diferente al 'numero_empleado'.
            $authUser = Auth::getUserByUsername($usuario);
            
            // Si encontramos al usuario en Auth, usamos su numero_empleado real. Si no, usamos el usuario como fallback.
            $numero_empleado_real = $authUser['numero_empleado'] ?? $usuario;
            $email = $authUser['email'] ?? $_SESSION['email'] ?? '';

            // 2. Buscar en la BD MySQL usando el numero_empleado REAL
            $sql_docente = "SELECT * FROM docentes WHERE numero_empleado = :num_emp OR numero_empleado = :usuario OR email = :email LIMIT 1";
            $stmt = $this->conn->prepare($sql_docente);
            $stmt->execute([
                ':num_emp' => $numero_empleado_real,
                ':usuario' => $usuario,
                ':email' => $email
            ]);
            $docente = $stmt->fetch();
            ...
            if (!$docente) {
                ...
                $msg = 'No se encontró su perfil de docente asociado. <br>' .
                    'Usuario Login: ' . htmlspecialchars($usuario) . '<br>' .
                    'Num. Empleado esperado: ' . htmlspecialchars($numero_empleado_real);
                $_SESSION['error'] = $msg;
